...
export default function EditFacultyPage() {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        ...
        setFormData({
          name: data.name || '',
          role: data.role || 'Faculty Advisor',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await apiMulti.put(`/faculty/${id}`, data);
      setSuccess(true);
      setTimeout(() => {
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <h2 className="text-3xl font-extrabold text-black mb-2">Faculty Updated!</h2>
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-black mb-2">Edit Faculty Advisor</h1>
            <p className="text-gray-500 font-medium text-lg">Update details for {formData.name}.</p>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div className="md:col-span-2 flex flex-col items-center justify-center">
               ...
               <div onClick={() => document.getElementById('image-upload').click()} className="w-40 h-40 bg-gray-50 border-2 border-dashed border-gray-200 rounded-full flex flex-col items-center justify-center cursor-pointer hover:border-[#FFB800] hover:bg-[#FFB800]/5 transition-all overflow-hidden relative group">
                 ...
                 <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white"><FiUpload size={24} /></div>
            ...
            <div className="md:col-span-1">
              ...
              <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#FFB800]/50 transition-all font-bold text-black" />
            </div>
            <div className="md:col-span-1">
              <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Role / Designation</label>
              <input type="text" required value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})} placeholder="e.g. Faculty Advisor" className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-[#FFB800]/50 transition-all font-bold text-black" />
            </div>
            <div className="md:col-span-1">
            ...
            <div className="md:col-span-2">
              ...
              <textarea rows={6} required value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-3xl focus:outline-none focus:ring-2 focus:ring-[#FFB800]/50 transition-all font-bold text-black resize-none" maxLength={1000} />
            </div>
          </div>
          <div className="flex gap-4">
            <button type="submit" disabled={saving} className="flex-1 flex items-center justify-center gap-2 bg-black text-white px-8 py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-sm hover:bg-[#FFB800] hover:text-black transition-all shadow-xl disabled:opacity-50">
            ...
            <button type="button" onClick={() => router.back()} className="px-8 py-5 bg-gray-100 text-gray-500 rounded-2xl font-black uppercase tracking-[0.2em] text-sm hover:bg-gray-200 transition-all">Cancel</button>
          </div>
